...
const CustomFieldSelection = ({ onHandleSelection }: ICustomFieldSelection) => {
  ...
  const submitNewField = () => {
    ...
    if (selectedType === CustomFieldLabel.BOOLEAN) {
      onHandleSelection(selectedType, {
        variableName: newFieldLabel,
        options: switchLabels.onLabel + ',' + switchLabels.offLabel,
      });
    } else {
        ...
        <div>
          <div className="pb-4 pt-4">
            ...
            <input
              ...
              placeholder="On"
              onChange={(event) => {
                onHandleOnLabelChange(event);
              }}
              name="on-label"
              value={switchLabels.onLabel}
            />
          </div>
          <div className="pb-4 pt-4">
            ...
            <input
              ...
              placeholder="Off"
              onChange={(event) => {
                onHandleOffLabelChange(event);
              }}
              name="off-label"
              value={switchLabels.offLabel}
            />
          </div>
        </div>
      )}
