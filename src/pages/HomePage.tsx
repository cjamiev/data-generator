...
import { IRandomField, NewCustomField } from "../atoms/NewCustomField";
...
const HomePage = () => {
  ...
  const updatePredefinedSelection = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const updatedSelection = matched ?
      predefinedSelection.filter(item => item !== selectedValue) : predefinedSelection.concat([selectedValue]);
  ...
  return (
    <>
      ...
      {showCustomFields && <div>
        <NewCustomField onHandleSelection={updateNewFieldSelection} />
        <button onClick={hideCustomFields}>Close</button>
      </div>}
      {showPredefinedFields && <div>
        <PredefinedFieldSelection predefinedSelection={predefinedSelection} onHandleSelection={updatePredefinedSelection} />
